...
def test_render_omits_rebuttal_line_when_empty():
    ...
    s["pessimists"]["NVDA"] = PessimistCase(
        ...
        claims=[],
        confidence=60,
        rebuttal="opponent has a point",
        counter_claims=[],
    )
    md = render_report(s, prior_outcomes=[])
    # Optimist section has no rebuttal, pessimist does. Check optimist-side absence.
    optimist_idx = md.index("🟢 긍정론자")
    pessimist_idx = md.index("🔴 비관론자")
    optimist_block = md[optimist_idx:pessimist_idx]
    assert "반박:" not in optimist_block
    pessimist_block = md[pessimist_idx:]
    assert "반박: opponent has a point" in pessimist_block
