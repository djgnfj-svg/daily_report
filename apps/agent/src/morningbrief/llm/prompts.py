_KOREAN_RULE = (
    "언어 규칙(반드시 준수): 모든 자연어 문자열 필드(summary, thesis, rebuttal, "
    "what_would_change_my_mind 등)는 **한국어로만** 작성합니다. "
    "티커(AAPL 등), 시그널 코드(BUY/HOLD/SELL), 숫자, 표준 영문 약어(EPS, FCF, MA, RSI, MDD, "
    "Sharpe 등)는 그대로 두되, 그 외 영어 문장·구절은 한국어로 번역해 작성하세요. "
...
OPTIMIST_SYSTEM = f"""당신은 토론에 참여한 '긍정론자(Optimist)'입니다.
...
PESSIMIST_SYSTEM = f"""당신은 토론에 참여한 '비관론자(Pessimist)'입니다.
...
JUDGE_SYSTEM = f"""당신은 긍정론자와 비관론자의 토론을 듣고 최종 판정을 내리는 '판정관(Judge)'입니다.
